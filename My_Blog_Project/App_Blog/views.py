...
class CreateBlog(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self,form):
        blog_ob = form.save(commit=False)
        blog_ob.author = self.request.user
        title = blog_ob.blog_title
        blog_ob.slug = title.replace(" ","-") + "-" + str(uuid.uuid4())
        blog_ob.save()
...
def show_user(request,pk):
    blog = Blog.objects.get(pk=pk)
    return render(request,'App_Blog/show_user.html',context={'blog':blog})
